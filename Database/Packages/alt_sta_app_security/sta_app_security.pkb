...
--     l_hashed_password := dbms_crypto.hash(utl_raw.cast_to_raw(pi_password || pi_salt), dbms_crypto.hash_sh256);
  ...
  if sta_usr.f_get_usr_by_email(pi_email => p_username).deleted_flg = 'Y' then
    return false;
  end if;
    --
...
--             end if;
  ...
  is
    ...
    cursor c_usr_auths( b_email               sta_user.email%type
                      ...
                      , b_read_only           sta_authorization.read_only%type
                      )
    is
      select 1
      ...
      and    coalesce(usr_auth.auth_read_only, 'N')         = coalesce(b_read_only, usr_auth.auth_read_only, 'N')
      ;
  ...
  begin
    ...
    apex_debug.message('pi_read_only          => '|| pi_read_only);
    -- debugging end
